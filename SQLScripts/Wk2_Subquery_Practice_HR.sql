...
-- step 1: all departments located at 1700
SELECT *
FROM departments 
WHERE location_id = 1700;

-- step 2: all employees in those departments
SELECT e.first_name, e.last_name
FROM employees e
WHERE e.department_id IN (SELECT d.department_id FROM departments d WHERE d.location_id = 1700)
ORDER BY e.first_name, e.last_name;

-- find all employees who do not locate in the location with the id 1700
SELECT e.first_name, e.last_name
FROM employees e
WHERE e.department_id not IN (SELECT d.department_id FROM departments d WHERE d.location_id = 1700)
ORDER BY e.first_name, e.last_name;
...
-- find all employees whose salaries are greater than the average salary of all employees
SELECT e1.first_name, e1.last_name, e1.salary
FROM employees e1
WHERE e1.salary > (SELECT avg(e2.salary) FROM employees e2);

-- find all departments which have atleast one employee with salary greater than 10000
SELECT d.department_name
FROM departments d
WHERE EXISTS (SELECT * FROM employees e WHERE e.salary > 10000 AND e.department_id = d.department_id)
ORDER BY d.department_name;

-- find all departments which have atleast one employee with salary greater than 10000
SELECT d.department_name
FROM departments d
WHERE not EXISTS (SELECT * FROM employees e WHERE e.salary > 10000 AND e.department_id = d.department_id)
ORDER BY d.department_name;
