...
-- SET MODE -- 
-- http://stackoverflow.com/questions/11448068/mysql-error-code-1175-during-update-
-- in-mysql-workbench
...
update employee set superior_emp_id =
 (select emp_id from emp_tmp where Last_Name = 'Hawthorne' and First_Name = 
'Susan')
...
update employee set superior_emp_id =
 (select emp_id from emp_tmp where Last_Name = 'Markham' and First_Name = 
'Theresa')
...
-- 2. For each branch, list the branch name and city, plus the last name and title 
...
-- 4. Show the last name and title of each employee, along with the last name and title 
-- of that employee's boss.
SELECT e.LAST_NAME AS 'Employee\'s Last Name', 
		e.TITLE AS 'Employee\'s Title', 
		b.LAST_NAME AS 'Boss\'s Last Name', 
		b.TITLE AS 'Boss\'s Title'
FROM employee e INNER JOIN employee b ON e.SUPERIOR_EMP_ID = b.EMP_ID;

-- 5. For each account, show the name of the account's product, the available balance, and 
...
FROM account a INNER JOIN product p ON a.PRODUCT_CD = p.PRODUCT_CD
					...
					INNER JOIN individual i ON i.CUST_ID = c.CUST_ID;

-- 6. List all account transaction details for individual customers whose last name starts with 'T'.
SELECT a_t.*
FROM acc_transaction a_t INNER JOIN account a ON a_t.ACCOUNT_ID = a.ACCOUNT_ID
