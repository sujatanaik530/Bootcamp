		...
		while (choice != 7) {
			...
			switch (choice) {
				...
				case 5:
					...
					Item itemToRemove = cart.getItemCollection().get(item_name);
					Integer quantityMore = itemToRemove.getQuantity();
					if (cart.remove(item_name) != null) {
						System.out.println(item_name + " was removed from the cart");
						// remove from cart and add back to app system
						Item itemToUpdate = app.getItemCollection().get(item_name);
						if (itemToUpdate != null) {
							// items of the same kind were still in the app system
							// just add back what was taken off the cart
							Integer quantityOld = itemToUpdate.getAvailableQuantity();
							itemToUpdate.setAvailableQuantity(quantityOld + quantityMore);
							app.getItemCollection().replace(item_name, itemToUpdate);
						}
						else {
							// item had been removed from app system because it was all added to cart
							// put it back in the app system
							itemToUpdate = itemToRemove;
							itemToUpdate.setAvailableQuantity(quantityMore);
							app.getItemCollection().put(item_name, itemToUpdate);
						}
					} else {
